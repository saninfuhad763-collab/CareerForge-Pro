                ...
                {report.hasEvidence ? (
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <div id="requirements-evidence-list" role="tabpanel" aria-labelledby={`tab-${activeTab}`} className="space-y-3 max-h-72 overflow-y-auto pr-1 pb-4">
                        {filteredEvidence.map((item, idx) => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-3.5 rounded-xl border text-xs shadow-xs transition-all ${
                                isExact
                                  ? 'bg-emerald-50/35 dark:bg-emerald-950/25 border-emerald-300/80 dark:border-emerald-800/80'
                                  : isAlias
                                  ? 'bg-sky-50/35 dark:bg-sky-950/25 border-sky-300/80 dark:border-sky-800/80'
                                  : isPartial
                                  ? 'bg-amber-50/35 dark:bg-amber-950/25 border-amber-300/80 dark:border-amber-800/80'
                                  : 'bg-rose-50/35 dark:bg-rose-950/25 border-rose-300/80 dark:border-rose-800/80'
                              }`}
                            >
                              ...
                              {isPartial && (
                                <div className="mt-2 space-y-1.5">
                                  ...
                                  {item.gapAnalysis && (
                                    <div className="text-[10px] text-amber-800 dark:text-amber-300 bg-amber-100/40 dark:bg-amber-950/40 p-2 rounded-lg border border-amber-200/40 dark:border-amber-900/40 flex items-start gap-1.5">
                                      <AlertTriangle className="w-3 h-3 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                                      <div>
                                        <span className="font-bold">Missing Context: </span>
                                        {item.gapAnalysis}
                                      </div>
                                    </div>
                                  )}
                                  {item.remediationTip && (
                                    <div className="text-[10px] text-indigo-700 dark:text-indigo-300 bg-indigo-50/50 dark:bg-indigo-950/40 p-2 rounded-lg border border-indigo-100 dark:border-indigo-900/40 flex items-start gap-1.5">
                              ...
                              {isMissing && (
                                <div className="mt-2 text-[10px] text-rose-700 dark:text-rose-400 flex items-center justify-between">
                                  <span className="flex items-center gap-1">
                                    <AlertCircle className="w-3 h-3 text-rose-500" />
                                    <span>No evidence found across experience, skills, or projects.</span>
                                  </span>
                                  <button
                                    type="button"
                                    onClick={() => {
                      ...
                      {/* Overflow gradient fade cue */}
                      {filteredEvidence.length > 2 && (
                        <div
                          className="pointer-events-none absolute bottom-0 inset-x-0 h-6 bg-gradient-to-t from-white dark:from-slate-900 to-transparent rounded-b-xl"
                          aria-hidden="true"
                        />
                      )}
                    </div>
